    ...
    def solve(self):
        while self.epoch <= self.cfg.max_epoch:
            ...
            ckpt_path = os.path.join(
                self.cfg.ckpt_dir, 'epoch_' + str(self.epoch) + '.pth')
            torch.save({
...
def patch_visualize(model, dataloader, save_path):
    model.eval()
    tot_samples = 50
    time_all = 0
    samples = []
    ttt = 0
    with tqdm(total=tot_samples) as t:
        for i, data in enumerate(dataloader):
            ...
            samples.append(end_points['vis_list'])

            t.set_description(
                "Test [{}/{}]: ".format(i+1, tot_samples)
            )
    ...
    with open(f"{save_path}/samples_list.pkl", 'wb') as f:
