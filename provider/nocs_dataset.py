...
#         #---使用SuperPoint提取关键点
#         # self.kpt_extractor = SuperPointFrontend(weights_path="./model/superpoint/superpoint_v1.pth", 
#         #                                         nms_dist=1,
#         #                                         conf_thresh=0.,
#         #                                         nn_thresh=0.7,
#         #                                         cuda=False,
#         #                                         kpt_num=96,
#         #                                         )
#         # for param in self.kpt_extractor.net.parameters():
...
#         rgb = rgb[rmin:rmax, cmin:cmax, :]
#         rgb4sp = rgb.copy()
#         rgb4sp = self.transform(rgb4sp)
#         rgb = cv2.resize(rgb, (self.img_size, self.img_size), interpolation=cv2.INTER_LINEAR) 
#         # data augmentation
#         rgb = self.colorjitter(Image.fromarray(np.uint8(rgb)))
#         rgb = np.array(rgb)
#         rgb = rgb_add_noise(rgb)
#         rgb = self.transform(rgb) # (C = 3, H, W)
...
#         else:
#             choose_idx = np.random.choice(len(choose), self.sample_num, replace=False)
#         choose = choose[choose_idx] # (N, )

#         #---利用SuperPoint从RGB中提取关键点,并加入choose中
#         # gray_img = 0.299 * rgb4sp[0:1, :, :] + 0.587 * rgb4sp[1:2, :, :] + 0.114 * rgb4sp[2:3, :, :]
#         # kpts = self.kpt_extractor.run(gray_img) # (B = 1, n, 3)
#         # kpts_choose = kpts.squeeze(0).numpy()[:, :2] # (n, 2)，2是像素坐标维度
#         # kpts_choose_idx = (kpts_choose[:, 0:1] - 1) * (cmax - cmin) + kpts_choose[:, 1:2] # (n, 1)
#         # kpts_choose_idx = kpts_choose_idx.reshape(kpts_choose_idx.shape[0])
#         # choose = np.append(choose, kpts_choose_idx)
#         #---
...
#         #---只在利用SuperPoint提取关键点时使用
#         # ret_dict['kpt_choose'] = kpts_choose_idx
#         #---
