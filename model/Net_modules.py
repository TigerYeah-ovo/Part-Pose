...
#------------------------------------------
class GAFAStructureLikeFeatureAggregator(nn.Module):
    def __init__(self, cfg):
        super().__init__()
...
class NOCS_Predictor(nn.Module):
    def __init__(self, cfg):
        ...
        self.nocs_mlp = nn.Sequential(
            nn.Conv1d(cfg.kpt_feat_dim, cfg.mlp_output_dim, 1), # 原始第二个参数为第一个参数的二倍
            nn.BatchNorm1d(cfg.mlp_output_dim),
            nn.ReLU(),
            nn.Conv1d(cfg.mlp_output_dim, cfg.mlp_output_dim, 1),
            nn.BatchNorm1d(cfg.mlp_output_dim),
            nn.ReLU(),
            nn.Conv1d(cfg.mlp_output_dim, cfg.kpt_feat_dim, 1),
            nn.BatchNorm1d(cfg.kpt_feat_dim),
        )
        
        self.self_attn_layer = SelfAttnLayer(cfg.AttnLayer)
        
        self.atten_mlp = nn.Sequential(
            nn.Conv1d(cfg.kpt_feat_dim, cfg.mlp_output_dim, 1),
            nn.ReLU(),
            nn.Conv1d(cfg.mlp_output_dim, cfg.mlp_output_dim, 1),
            nn.ReLU(),
            nn.Conv1d(cfg.mlp_output_dim, self.cat_num*3*self.bins_num, 1), 
        )
...
class PoseSizeEstimator(nn.Module):
    def __init__(self, arg):
        ...
        self.pose_mlp1 = nn.Sequential(
            nn.Conv1d(64 + 64 + arg.kpt_feat_dim, 512, 1),
            nn.ReLU(),
            nn.Conv1d(512, 256, 1),
            nn.ReLU(),
        )
...
class FPS():
    ...
    def forward(self, pts, rgb_local, pts_local):
        ...
        Args:
            ...
            pts_local (_type_): (b, c, n)
        """
        pts_index = self.farthest_point_sample(pts, self.kpt_num)
        kpt_3d = self.index_points(pts, pts_index)
        kpt_feature = self.index_points(torch.cat((pts_local, rgb_local), dim=1).transpose(1, 2), pts_index)
        kpt_point_feature = self.index_points(pts_local.transpose(1, 2), pts_index)
        return kpt_3d, kpt_feature, kpt_point_feature
...
class RGBGuidedKeypointSelector(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.kpt_num = cfg.kpt_num
        self.query_dim = cfg.query_dim
...
class QuadraticLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Quadratic part
        quad_output = torch.einsum('Bbi,ioj,Bbj->Bbo', x, self.quad_weights, x)

        return (linear_output + quad_output).transpose(1, 2)
#------------------------------