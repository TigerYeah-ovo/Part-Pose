...
from model.Net_modules import InstanceAdaptiveKeypointDetector, GeometricAwareFeatureAggregator, PoseSizeEstimator, NOCS_Predictor, Reconstructor, FPS, QuadraticLayer, RGBGuidedKeypointSelector
...
# from model.superpoint.extractor import SuperPointFrontend
...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.rgb_backbone == "resnet":
            ...
            # frozen dino
            self.rgb_extractor = torch.hub.load('YOUR_REPO_DIR/dinov2-main', 'dinov2_vits14', source='local', pretrained=False)
            self.rgb_extractor.load_state_dict(torch.load('YOUR_PARA_DIR/dinov2_vits14_pretrain.pth'))
        ...
        elif cfg.kpt_select_module == "RGKS":
            self.RGKS = RGBGuidedKeypointSelector(cfg.RGKS)
        #     self.RGKS = SuperPointFrontend(weights_path="./model/superpoint/superpoint_v1.pth",
        #                                    nms_dist=1,
        #                                    conf_thresh=0.,
        #                                    nn_thresh=0.7,
        #                                    cuda=True
        #                                    )
        #     for param in self.RGKS.net.parameters():
        #         param.requires_grad = False
        # elif cfg.kpt_select_module == "SP":
        #     None
        else:
            raise NotImplementedError
    ...
    def forward(self, inputs):
        ...
        choose = choose.unsqueeze(1).repeat(1, d, 1) # (B, C = 128, n = 1024)
        ...
        # 这个分支要产生kpt_3d, kpt_feature, kpt_point_feature
        if self.kpt_selector == "IAKD":
        ...
        elif self.kpt_selector == "RGKS__":
            batch_kpt_query, heat_map = self.RGKS(rgb_local, pts_local)
            # (B, N = 96, 3)
            kpt_3d = torch.bmm(heat_map, pts)
            # (B, N = 96, F = 256)
            kpt_feature = torch.bmm(heat_map, torch.cat((pts_local, rgb_local), dim=1).transpose(1, 2))
            ...
            #需要与前面提取出的关键点保持一致
            #----------------------------------------------------------------
        # 直接将1024个choose出来的像素作为输入，使用SuperPoint提关键点
        elif self.kpt_selector == "RGKS":
            choose_rgb = inputs["choose"]
            g_rgb = rgb.contiguous().view(b, 3, -1)
            choose_rgb = choose_rgb.unsqueeze(1).repeat(1, 3, 1) # (B, 3, n = 1024)
            chosen_rgb = torch.gather(g_rgb, 2, choose_rgb).contiguous() # (B, 3, n)
            gray_img = 0.299 * chosen_rgb[:, 0:1, :] + 0.587 * chosen_rgb[:, 1:2, :] + 0.114 * chosen_rgb[:, 2:3, :]
            gray_img = gray_img.squeeze(1)
            gray_img = gray_img.contiguous().view(gray_img.shape[0], 32, 32)
            heat_map = self.RGKS.run(gray_img) # (B, n = 96, 3), (x, y, conf)
            heat_map = heat_map[:, :, :3].contiguous().view(b, self.cfg.IAKD.kpt_num, -1)
        else:
            raise NotImplementedError
        ...
        if self.training:
            ...
            end_points['vis_list'] = output['vis_list']
...
class Loss(nn.Module):
    ...
    def forward(self, endpoints):
        ...
        loss_all = self.cfg.pose*loss_pose + self.cfg.nocs*loss_nocs + self.cfg.cd*loss_cd + self.cfg.recon*loss_recon + self.cfg.delta*loss_delta
