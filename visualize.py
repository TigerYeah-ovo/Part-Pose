...
def init():
    ...
    cfg.draw_dir = os.path.join(log_dir, "viz_results")
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader)):
            # path = data['path'][0]
            path = dataloader.dataset.result_pkl_list[i]
            inputs = {
                    'rgb': data['rgb'][0].cuda(),
                    'pts': data['pts'][0].cuda(),
            ...
            image_path = os.path.join("/data/yecl24/Datasets/NOCS", data['image_path'])
