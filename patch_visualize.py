...
from utils.solver import patch_visualize, get_logger
from provider.nocs_dataset import TestDataset
...
if __name__ == "__main__":
    ...
    if cfg.setting == 'supervised':
        save_path = os.path.join(cfg.log_dir, 'eval_epoch' + str(cfg.test_epoch) + '_patch_visualize')
        setting = 'supervised'
    else:
        if cfg.mask_label:
            save_path = os.path.join(cfg.log_dir, 'eval_withMaskLabel_epoch' + str(cfg.test_epoch) + '_patch_visualize')
            setting = 'unsupervised_withMask'
        else:
            save_path = os.path.join(cfg.log_dir, 'eval_woMaskLabel_epoch' + str(cfg.test_epoch) + '_patch_visualize')
    ...
    checkpoint_dir = os.path.join(cfg.ckpt_dir, 'epoch_' + str(cfg.test_epoch) + '.pth')
    checkpoint = torch.load(checkpoint_dir)
    logger.info("=> loading checkpoint from path: {} ...".format(checkpoint_dir))
    ...
    patch_visualize(model, dataloder, save_path)
